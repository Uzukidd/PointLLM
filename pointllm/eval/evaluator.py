...
class OpenAIOpenFreeFormClsEvaluator:
    def __init__(self, inputs, output_dir, output_file, model_type="gpt-4-0613"):
        ...
        self.price_1k_completion_tokens = GPT_PRICES[model_type][
        ...
        self.gpt_prompt = (
            chatgpt_open_free_from_cls_prompt
            if "gpt-3.5" in model_type
            else gpt4_open_free_from_cls_prompt
        )
    ...
    def check_model_type(self):
        ...
        if "gpt-4" not in self.model_type:
            print(
                f"[WARNING] You are using {self.model_type} for evaluation. We recommend using gpt-4 for this task."
            )
    ...
    def evaluate_result(self, result):
        ...
        messages = [
            {
                "role": "user",
                "content": self.gpt_prompt.format(
            ...
            with Pool(num_workers) as pool:
                with tqdm(total=len(results)) as pbar:  # create a progress bar
                    for (
                        ...
                        prompt_tokens,
                        completion_tokens,
                    ) in pool.imap_unordered(self.evaluate_result, results):
                        self.correct_predictions += accuracy
                        self.total_predictions += 1
                        self.prompt_tokens += prompt_tokens
                        self.completion_tokens += completion_tokens
    ...
    def save_results(self, is_temp=False):
        ...
        if self.total_predictions - self.invalid_responses == 0:
            ...
            accuracy = (
                ...
                / (self.total_predictions - self.invalid_responses)
                * 100
            )
    ...
    def get_costs(self):
        return (
            self.prompt_tokens * self.price_1k_prompt_tokens / 1000
            + self.completion_tokens * self.price_1k_completion_tokens / 1000
        )


class OpenAICloseSetClsEvaluator(OpenAIOpenFreeFormClsEvaluator):
    def __init__(
        self, inputs, output_dir, output_file, model_type="gpt-3.5-turbo-0613"
    ):
        super().__init__(inputs, output_dir, output_file, model_type)
        self.gpt_prompt = (
            chatgpt_close_set_cls_prompt
            if "gpt-3.5" in model_type
            else gpt4_close_set_cls_prompt
        )
        ...
        try:
            ...
            catfile = os.path.join(
                ...
                "../data/modelnet_config/modelnet40_shape_names_modified.txt",
            )
            self.candidate_lists_names = [
                line.strip() for line in open(catfile)
        ...
        # * make the prompt
        candidate_lists = [
            f"{i}: {cat}" for i, cat in enumerate(self.candidate_lists_names)
        ]
        self.num_categories = len(candidate_lists)
        self.candidate_lists = "\n".join(candidate_lists)
        self.gpt_prompt = (
            self.gpt_prompt.format(
                num_categories=self.num_categories, candidate_lists=self.candidate_lists
            )
            + "{model_output}\nOutput: "
        )
    ...
    def resume_processing(self):
        ...
        if os.path.exists(processed_results_path):
            ...
            self.total_predictions = saved_results["total_predictions"]
            self.invalid_responses = saved_results["invalid_responses"]
            self.invalid_correct_predictions = saved_results[
    ...
    def evaluate_result(self, result):
        ...
        messages = [
            {
                "role": "user",
                "content": self.gpt_prompt.format(model_output=model_output),
            }
        ]
        ...
        accuracy, cls_result, cls_label, reason = self.parse_gpt_response_evaluate(
            gpt_response, ground_truth
        )  # return 0, "INVALID", gpt_response if not valid
    ...
    def save_results(self, is_temp=False):
        ...
        if self.total_predictions - self.invalid_responses == 0:
            ...
            clean_accuracy = (
                (self.correct_predictions - self.invalid_correct_predictions)
                / (self.total_predictions - self.invalid_responses)
                * 100
            )
        with open(output_path, "w") as f:
...
class OpenAIObjectCaptioningEvaluator(OpenAIOpenFreeFormClsEvaluator):
    def __init__(self, inputs, output_dir, output_file, model_type="gpt-4-0613"):
        super().__init__(inputs, output_dir, output_file, model_type)
        self.gpt_prompt = (
            chatgpt_object_captioning_prompt
            if "gpt-3.5" in model_type
            else gpt4_object_captioning_prompt
        )
    ...
    def evaluate_result(self, result):
        ...
        gpt_score, reason = self.parse_gpt_response_evaluate(
            gpt_response, ground_truth
        )  # return 0, "INVALID", gpt_response if not valid
    ...
    def save_results(self, is_temp=False):
        ...
        if self.total_predictions - self.invalid_responses == 0:
            ...
            average_score = self.total_scores / (
                self.total_predictions - self.invalid_responses
            )
        with open(output_path, "w") as f:
            results_to_save = {
...
def start_evaluation(
    ...
    if eval_type == "open-free-form-classification":
        evaluator = OpenAIOpenFreeFormClsEvaluator(
            results, output_dir, output_file, model_type=model_type
        )
    elif eval_type == "modelnet-close-set-classification":
        evaluator = OpenAICloseSetClsEvaluator(
            results, output_dir, output_file, model_type=model_type
        )
    elif eval_type == "object-captioning":
        evaluator = OpenAIObjectCaptioningEvaluator(
            results, output_dir, output_file, model_type=model_type
        )
    elif eval_type == "localllama-modelnet-close-set-classification":
        from pointllm.eval.localLLaMA_evaluator import (
            localLLaMA_close_set_cls_evaluator,
        )

        evaluator = localLLaMA_close_set_cls_evaluator(results, output_dir, output_file)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_path", type=str, default="", help="Path to the results file."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=True,
        action="store_true",
        help="Whether to use parallel evaluation.",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        choices=[
            "modelnet-close-set-classification",
            "open-free-form-classification",
            "object-captioning",
        ],
        default="object-captioning",
    )
    ...
    output_file = os.path.basename(args.results_path).replace(
        ".json", f"_evaluated_{args.model_type}.json"
    )

    # if exists, then exit
    if os.path.exists(os.path.join(args.output_dir, output_file)):
        print(
            f"[INFO] Evaulated results already exists in {os.path.join(args.output_dir, output_file)}."
        )
        exit()
